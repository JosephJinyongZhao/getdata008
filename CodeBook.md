The sample is from "Human Activity Recognition Using Smartphones Dataset" conducted by Smartlab - Non Linear Complex Systems Laboratory(www.smartlab.ws).
...
# Proceeded data:
the training and the test sets are merged into one data set. Only the measurements on the mean and standard deviation for each measurement. Activities are coded as character with their descriptive names. Data set variables have their descriptive variable names as the raw data set. The data content is the average of each variable for each activity and each subject.
